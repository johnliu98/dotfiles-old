...
keypress(XKeyEvent *ev)
{
	...
	if (ev->state & ControlMask) {
		...
		case XK_h: if (columns > 1) ksym = XK_Left; else ksym = XK_Prior;    break;
		case XK_j: ksym = XK_Down;  break;
		case XK_k: ksym = XK_Up; break;
		case XK_l: if (columns > 1) ksym = XK_Right; else ksym = XK_Next;  break;
		default:
			return;
		}
	}
...
usage(void)
{
	fputs("usage: dmenu [-bcfiPrv] [-g columns] [-l lines] [-wd width] [-p prompt] [-fn font]\n"
	      "             [-m monitor] [-nb color] [-nf color] [-sb color] [-sf color] [-w windowid]\n", stderr);
	exit(1);
}
